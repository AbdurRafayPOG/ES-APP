...
class _EmergencyStatusPageState extends State<EmergencyStatusPage> {
  ...
  void _startAssignedListener() {
    ...
    _assignedListener = _database
        ...
        .listen((event) {
          ...
          // 🔥 Emergency removed from assigned
          if (!snapshot.exists) {
            print("📋 Emergency removed from assigned");
            
            // Check if it moved to SOS_Done (completed)
          ...
          // 🔥 Emergency still exists - update status
  ...
  // ============================================================
  // 🔥 PERIODIC RESPONDER STATUS CHECK
  // ============================================================
  void _startResponderStatusCheck() {
    ...
    _responderStatusTimer = Timer.periodic(const Duration(seconds: 5), (timer) async {
      ...
      try {
        ...
        // 🔥 Check if responder is truly offline
        bool isOffline = false;
        
        if (status == 'inactive' || status == 'Inactive') {
          isOffline = true;
        }
        // 🔥 If busy with THIS emergency - KEEP ACTIVE
        else if (status == 'busy' && currentEmergencyId == widget.emergencyId) {
          return;
        }
        // 🔥 If busy with different emergency - offline
        else if (status == 'busy' && currentEmergencyId != widget.emergencyId) {
          isOffline = true;
        }
        // 🔥 If not busy, check active/online
        else if (status != 'busy') {
        ...
        // 🔥 Also check if emergency still exists
        final assignedSnapshot = await _database
            .child('assigned')
            .child(_responderId!)
            .child(widget.emergencyId)
  ...
  // ============================================================
  // 🔥 HANDLE EMERGENCY COMPLETED - Navigate to NavBar (which has SOS page)
  // ============================================================
  void _handleEmergencyCompleted() {
    ...
    // 🔥 Short notification with icon
    _showNotification(
      title: 'Emergency Completed',
      message: 'Responder has marked this emergency as done.',
  ...
  // ============================================================
  // 🔥 HANDLE EMERGENCY NOT FOUND - Navigate to NavBar (which has SOS page)
  // ============================================================
  ...
  // ============================================================
  // 🔥 HANDLE RESPONDER OFFLINE - Navigate to NavBar (which has SOS page)
  // ============================================================
  ...
  // ============================================================
  // 🔥 NAVIGATE TO NavBar (which has SOS page + Bottom Nav)
  // ============================================================
  void _goToNavBar() {
    Future.delayed(const Duration(milliseconds: 600), () {
      if (mounted) {
        // ✅ Navigate to NavBar which contains the SOS page with bottom nav
  ...
  Future<void> _copyEmergencyId() async {
    if (widget.emergencyId.isEmpty) {
      Get.snackbar(
        'Error',
        'No Emergency ID available',
        snackPosition: SnackPosition.TOP,
    ...
    try {
      await Clipboard.setData(ClipboardData(text: widget.emergencyId));
      Get.rawSnackbar(
        message: 'Copied!',
        snackPosition: SnackPosition.TOP,
  ...
  // ============================================================
  // 🔥 UPDATED VIDEO CALL - Allow when busy with THIS emergency
  // ============================================================
  void _startVideoCall() async {
    ...
    String userName = Keys.userName.isNotEmpty ? Keys.userName : 'User';
  ...
  Widget _buildTimeInfoCard() {
    bool isCompleted = _status == 'completed' || _status == 'cancelled';
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(
                        _responderData?['UserName'] ?? 'Unknown',
                        style: const TextStyle(
                          fontSize: 16,
                          fontWeight: FontWeight.w600,
                          color: Colors.white,
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(
                        _responderData?['UserType'] ?? 'Responder',
                        style: const TextStyle(
                          fontSize: 16,
                          fontWeight: FontWeight.w600,
                          color: Colors.white,
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: (_status == 'completed' ? Colors.green : Colors.orange).withOpacity(0.2),
                          borderRadius: BorderRadius.circular(12),
                          border: Border.all(
                            color: (_status == 'completed' ? Colors.green : Colors.orange).withOpacity(0.3),
                          ),
                        ),
                        child: Text(
                          _status == 'completed' ? 'Completed' : 'En Route',
                          style: TextStyle(
                            fontSize: 14,
                            fontWeight: FontWeight.w600,
                            color: _status == 'completed' ? Colors.green : Colors.orange,
