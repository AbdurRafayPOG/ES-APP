...
class _ProfileFormWidgetState extends State<ProfileFormWidget> {
  ...
  Future<void> _fetchCurrentAddress() async {
    ...
    try {
      ...
      DatabaseEvent event = await ref.child(user.uid).once();
      ...
      if (snapshot.value != null) {
        ...
        // Immediately show saved address if available
      ...
      try {
        ...
        if (placemarks.isNotEmpty) {
          ...
          // Update address with fresh location
          setState(() {
        ...
        // Keep showing saved address, just mark as done
        setState(() {
          _isLoadingAddress = false;
          _hasFetchedLocation = true;
        });
    ...
    return StreamBuilder(
      ...
      builder: (context, AsyncSnapshot snapshot) {
        if (snapshot.hasData && snapshot.data.snapshot.value != null) {
          ...
          final userName = map['UserName'] ?? 'Not set';
          final userEmail = map['email'] ?? user.email ?? 'Not set';
          final userPhone = map['Phone'] ?? 'Not set';
          
          // If address changes from Firebase, update it
          String firebaseAddress = map['address'] ?? '';
          if (firebaseAddress.isNotEmpty && _address != firebaseAddress && !_isLoadingAddress) {
            WidgetsBinding.instance.addPostFrameCallback((_) {
              setState(() {
                _address = firebaseAddress;
              });
            });
          }
