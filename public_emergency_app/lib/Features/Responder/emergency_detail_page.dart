...
class EmergencyDetailPage extends StatelessWidget {
  ...
  // ✅ FIXED: _startVideoCall() with navigation to LiveStreamingPage
  ...
  // ❌ DO NOT OPEN LiveStreamingPage HERE
  // ZEGO invitation flow should handle the call page lifecycle.

  await callController.sendInvitation(
    calleeID: userID,
    calleeName: userName,
    callID: emergencyKey,
    inviterID: currentResponderId.isNotEmpty ? currentResponderId : 'responder',
    inviterName: "$responderName ($responderType)",
    isInviterHost: true,
    customData: {
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildInfoItem(
                          label: 'Coordinates',
                          value: '${emergencyData['userLat']?.toString() ?? 'N/A'}, ${emergencyData['userLong']?.toString() ?? 'N/A'}',
                          icon: Icons.location_on_rounded,
                          iconColor: Color(color),
                          isCopyable: true,
                          onCopy: () {
                            String lat = emergencyData['userLat']?.toString() ?? '';
                            String long = emergencyData['userLong']?.toString() ?? '';
                            if (lat.isNotEmpty && long.isNotEmpty && lat != 'N/A' && long != 'N/A') {
                              _copyToClipboard('$lat, $long', 'coordinates');
                            }
                          },
                        ),
                        _buildDivider(),
