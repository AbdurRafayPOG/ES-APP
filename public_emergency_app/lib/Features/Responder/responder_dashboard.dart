...
class _ResponderDashboardState extends State<ResponderDashboard> {
  ...
  // Dashboard Title - Dynamic based on user type
  ...
  // Cache for emergency data to prevent frequent reloads
  Map<dynamic, dynamic>? _cachedEmergencyData;
  String? _cachedEmergencyKey;

  // Track which emergency is being processed
  ...
  void initState() {
    ...
    // 🔥 Clear saved preference on app start
    _clearSavedPreferences();
    
    if (user != null) {
      _initializeFirebaseOnce().then((_) {
        _updateDashboardTitle();
        _verifyToggleState();
        _startPresenceTracking();
        _startResponderStatusCheck(); // ⬅️ ADD THIS LINE
      });
    } else {
      _isLoading = false;
    }
  ...
  // ============================================================
  // RESPONDER STATUS CHECK - Auto-remove emergencies when inactive or logged out
  // ============================================================
  void _startResponderStatusCheck() {
    ...
    _responderStatusCheckTimer = Timer.periodic(const Duration(seconds: 5), (timer) async {
        ...
        try {
          ...
          if (snapshot.value != null) {
            ...
            for (var emergencyKey in data.keys) {
              ...
              // If emergency data exists, move back to pending
      ...
      try {
        ...
        // 🔥 Check if responder is inactive or offline
        ...
        // 🔥 If inactive AND has an emergency assigned, remove it
        if (isInactive && currentEmergencyId.isNotEmpty) {
          ...
          if (emergencyData.isNotEmpty) {
            ...
            // Add back to pending emergencies
          }
  ...
  Future<void> _loadSwitchValue() async {
    ...
    try {
      ...
      // 🔥 CRITICAL: Read from Firebase FIRST
      ...
      try {
        ...
        if (responderSnapshot.value != null) {
          ...
          // Read the actual Firebase status
      ...
      // 🔥 IMPORTANT: If Firebase says inactive, force toggle OFF
      if (!firebaseActive) {
        ...
        // Save OFF to SharedPreferences to keep in sync
        await prefs.setBool('switchValue', false);
        // Ensure Firebase is set to inactive
        await _setResponderInactive();
        return;
      }
      
      // Firebase says active - use saved preference or default to false
      if (!mounted) return;
      setState(() {
  ...
  // ============================================================
  // ✅ UPDATE RESPONDER STATUS TO ACTIVE (After emergency done)
  // ============================================================
  ...
  Future<void> _initializeFirebaseOnce() async {
    ...
    try {
      ...
      // 🔥 FIRST: Sync Firebase to ensure correct state
      await _syncFirebaseState();
      
      // 🔥 THEN: Load toggle value based on Firebase state
  ...
  Future<void> _syncFirebaseState() async {
    ...
    try {
      ...
      if (responderSnapshot.value != null) {
        ...
        // If status is inactive, ensure all fields are correctly set
        ...
        // User doesn't exist in Firebase yet - create with inactive state
  ...
  // ============================================================
  // VERIFY TOGGLE STATE - New Method
  // ============================================================
  ...
  // ============================================================
  // CLEAR SAVED PREFERENCES - New Method
  // ============================================================
  Future<void> _clearSavedPreferences() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.remove('switchValue'); // Remove only the switch value
  ...
  // ============================================================
  // PRESENCE TRACKING - UPDATED METHODS
  // ============================================================

  /// Start presence tracking - updates every 10 seconds
  void _startPresenceTracking() {
    // 🔥 Only start presence tracking if toggle is ON
    if (!_switchValue) {
    ...
    _presenceTimer = Timer.periodic(const Duration(seconds: 10), (timer) {
      // 🔥 Check if toggle is still ON before updating
      if (_switchValue) {
        _updatePresence(true);
      } else {
        // If toggle turned OFF, stop tracking
        _presenceTimer?.cancel();
        _presenceTimer = null;
        _updatePresence(false);
      }
  ...
  /// Update responder presence in Firebase
  Future<void> _updatePresence(bool isOnline) async {
    if (user == null || myResponderRef == null) return;
    
    // 🔥 CRITICAL: Only update presence if the toggle is ON
    // If toggle is OFF, don't mark as online
  ...
  // ============================================================
// LOGOUT DIALOG - Fixed with no double loading
// ============================================================
                ...
                Expanded(
                  child: ElevatedButton(
                    onPressed: () {
                      ...
                      Get.back();
                      // 🔥 Perform logout without showing loading
                      _performLogout();
                    },
                    style: ElevatedButton.styleFrom(
...
// ============================================================
// PERFORM LOGOUT - Background emergency removal with no loading
// ============================================================
void _performLogout() async {
  // 🔥 Get references BEFORE signing out
  final currentUser = FirebaseAuth.instance.currentUser;
  ...
  // 🔥 Step 1: Remove emergency in background (fire and forget)
  ...
  // 🔥 Step 2: Update responder status to inactive (fire and forget)
  ...
  // 🔥 Step 3: Sign out immediately (no loading)
  try {
  ...
  // ============================================================
  // ✅ MARK EMERGENCY AS DONE - ONLY RIGHT SWIPE (NO DELETE)
  // ============================================================
  Future<bool> _markEmergencyAsDone(String emergencyKey, Map emergencyData) async {
    try {
      ...
      // 🔥 FIX 1: Add small delay to ensure data sync
    await Future.delayed(const Duration(milliseconds: 500));
    
    // 🔥 FIX 2: Refresh responder data and validate
    ...
    if (!refreshSnapshot.exists) {
      ...
      if (userId.isNotEmpty) {
        ...
        if (userSnapshot.value != null) {
          ...
          userAddress = userData['Address']?.toString() ?? 'No Address';
        }
      }
      
      // ============================================================
      // ✅ SOS TIME - ONLY FROM sosTime
      // ============================================================
      int sosTimeMs = 0;
      ...
      // ============================================================
      // 🔥 2. Remove from responder's assigned list
      // ============================================================
      await assignedRef!.child(emergencyKey).remove();
      
      // ============================================================
      // 🔥 3. Remove from user side
      // ============================================================
      if (userId.isNotEmpty) {
      ...
      // ============================================================
      // 🔥 4. Update responder status back to ACTIVE
      // ============================================================
  ...
  // ============================================================
  // PREMIUM COMPLETE SNACKBAR WITH UNDO
  // ============================================================
  ...
  // ============================================================
  // ✅ NAVIGATE TO COMPLETED EMERGENCIES
  // ============================================================
  ...
  // ============================================================
  // ✅ BUILD HOME CONTENT - NO LOADING SCREENS
  // ============================================================
  Widget _buildHomeContent() {
    ...
    // Show dashboard immediately, even if not fully initialized
    // The StreamBuilder will handle loading states for data
    return Scaffold(
      appBar: AppBar(
        backgroundColor: const Color(0xFF0F4C5C),
        centerTitle: true,
      ...
      body: Column(
        children: [
          const SizedBox(height: 8),
          // Completion Badge - Swipe right to mark as done
          if (assignedRef != null)
            Dismissible(
  ...
  // ============================================================
  // ✅ BUILD EMERGENCY LIST - ONLY RIGHT SWIPE (NO LEFT SWIPE)
  // ============================================================
